...
plot(house.ts)
plot(log(house.ts))
plot(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))
sd(house.ts)
sd(log(house.ts))
sd(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))
```

Logaritmic and box cox transformation seems more or less the same, in both the variance compared to the untransformed variable is less.

```{r}
tsdisplay(house.ts)
tsdisplay(log(house.ts))
tsdisplay(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))
```

We decide to use box cox since it allows us tuning at its best parameter lambda.

###B)Checking if there is seasonality

```{r Periodgram}
spec.pgram(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))
```
...
```{r ACF and PACF}
acf2(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))
```
...
ggseasonplot(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))
ggmonthplot(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))
```
...
kpss.test(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))
```

H0:stationarity, we refuse stationarity.

```{r Augmented Dickey Fuller Test}
adf.test(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)),alternative = "stationary")
```
...
plot(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts))))
```
```{r 1 diff ACF and PACF}
acf2(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts))))
```

```{r 1 diff KPSS Test for Stationarity}
kpss.test(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts))))
```

H0:stationarity, we refuse stationarity.

```{r 1 diff Augmented Dickey Fuller Test}
adf.test(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts))),alternative = "stationary")
```
...
pp.test(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)),alternative = "stationary"))
```

H0:non-stationarity, we refuse non stationarity.

```{r 1 diff standard deviation}
sd(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts))))
```
...
plot(diff(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))))
```
```{r 2 diff ACF and PACF}
acf2(diff(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))))
```

```{r 2 diff KPSS Test for Stationarity}
kpss.test(diff(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))))
```

H0:stationarity, we accept stationarity.

```{r 2 diff Augmented Dickey Fuller Test}
adf.test(diff(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts))),alternative = "stationary"))
```

H0:non-stationarity,we refuse non-stationarity.

```{r 2 diff Phillips Perron Unit Root Test}
pp.test(diff(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts))),alternative = "stationary"))
```

H0:non-stationarity,we refuse non-stationarity.

```{r 2 diff standard deviation}
sd(diff(diff(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))))
```
...
Final check
```{r}
ndiffs(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)),alpha = 0.05)
```
...
```{r}
model.1=Arima(house.ts,order=c(2,1,0),seasonal=list(order=c(0,1,1), period=12))
model.2=Arima(house.ts,order=c(2,1,0),seasonal=list(order=c(3,1,0), period=12))
model.3=Arima(house.ts,order=c(2,1,0),seasonal=list(order=c(3,1,1), period=12))
model.4=Arima(house.ts,order=c(0,1,4),seasonal=list(order=c(3,1,0), period=12))
model.5=Arima(house.ts,order=c(0,1,4),seasonal=list(order=c(0,1,1), period=12))

```

Based on the AICc, the winner is model.3
```{r}
...
jarque.bera.test(model.3$residuals[-325])
```

Quite not sure about it 

### 3f
To forecast the timeseries we use the `forecast()` function for `model.3`.

```{r}
plot(forecast(model.3, h=12))
```
...
Here we see that `model.2` has the best predictive power because it has the lowest RMSE.

### 3h
We will have a look at the `auto.arima()` function to see if it can find a better parameter set.

```{r}
model.auto=auto.arima(log(house.ts))
model.auto
```

A model was found with (p,d,q) = (2,1,2) and (P,D,Q) = (1,1,2). The AIC is worse than our manual selected model, but it has better predictive power:
...
We see that, among others, `ar1` and `ma1` have a correlation higher than 0.8. This is bad. Now, secondly, let us look at the residuals. They should be independently identically and normally distributed. This is what the residuals look like:
...
```{r}
model.auto.res.noo = model.auto$residuals[-c(117,123)]
...
The Student's t-test tells us that we cannot accept the residuals mean to be 0. 
...
The Jarque Bera test tells us that we can accept that the skewness and kurtosis of the residuals are both 0 (like the normal distribution) if we use 0.05 as our p-value.
...
We conclude that the automatically selected ARIMA model does not suffice and we will therefore advise to select our manually chosen model `model.3`.
