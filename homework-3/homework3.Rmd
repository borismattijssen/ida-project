...
A model was found with (p,d,q) = (2,1,2) and (P,D,Q) = (1,1,2). The AIC about the same as our manual selected model and it has about the same predictive power:
...
We can observe that `model.1` and the automatically selected ARIMA model have about the same performance (in terms of AICc and RMSE). Also, both invalidate the requirements for the residuals. Therefore, we conclude that both models could be chosen equally well.
