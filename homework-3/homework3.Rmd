...
```
comments
Since the decomposition we decided to use is multiplicative the box cox transformation is the best fit.
...
```
comments Doe not seem to occur seasonality


```{r ACF and PACF}
acf2(BoxCox(house.ts, lambda = BoxCox.lambda(house.ts)))
```
comments time series not stationary x2
In the PACF there is only one initial spike and then the vaues'modulus decrease to zero. 
...
```
comments ther is NO seasonality since all the months look the same.
...
```
Standard deviation strongly decrease from befor than differencing.
